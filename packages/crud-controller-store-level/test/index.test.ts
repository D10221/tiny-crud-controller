...
import levelStore, { newLevel } from "../src";
...
const store = levelStore("things")(db);
...
describe(require("../package.json").name, () => {
    ...
    it("throws not found", async () => {
        expect(await store.find("a").catch((error: any) => error.name)).toMatch("NotFound");
    })
    ...
    it("more stores", async () => {
        ...
        const store2 = levelStore("moreThings")(db2);
        expect(await store.add("a", { name: "aaa" })).toBe(undefined);
